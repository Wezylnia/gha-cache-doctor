...
Current release: `1.0.0` | License: MIT | Target framework: .NET 10

`gha-cache-doctor` is the first stable release. If you like CI/CD tooling, static analysis, or shaving minutes off slow pipelines, there are good first issues ready for contributors.
...
The project is stable and ready for CI usage. See [docs/project-status.md](docs/project-status.md) and [docs/roadmap.md](docs/roadmap.md).
...
```bash
dotnet tool install --global gha-cache-doctor --version 1.0.0
...
- name: Install gha-cache-doctor
  run: dotnet tool install --global gha-cache-doctor --version 1.0.0
